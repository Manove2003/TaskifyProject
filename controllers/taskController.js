...
const getTasks = async (req, res) => {
  try {
    ...
    // add completed todo check list  count  to each task
...
const updateTaskCheckList = async (req, res) => {
  try {
    ...
    if (!task) return res.status(404).json({ message: "Task not found" });
    if (
      !task.assignedTo.includes(req.params._id) &&
      req.user.role !== "admin"
    ) {
      return res
        .status(403)
    ...
    //auto updated progress based on check list completion
    const completedCount = task.todoChecklist.filter(
      (item) => item.completed
    ).length;
...
//@desc Dashboard Data of (Specfic user )
//@route GET /api/tasks/user-dashboard-data
//@access private
const getUserDashboardData = async (req, res) => {
  try {
    const userId = req.user._id; // only fetch the data for logged-in user
